 ...
 *
 * Jelly motion engine ported from NuvioMobile (NuvioMedia, GPL-3.0)
 * https://github.com/NuvioMedia/NuvioMobile — composeApp/src/androidMain/.../core/ui/jelly/JellyMotion.kt
...
internal class JellyMotion(initialIndex: Int, count: Int) {
    ...
    fun select(index: Int, animate: Boolean = true) {
        ...
        if (animate) {
            ...
            velocity.snapTo(0.0)
            press.snapTo(pressTarget)
            scaleX.snapTo(shapeTarget)
            scaleY.snapTo(shapeTarget)
            panel.snapTo(0.0)
            trackY.snapTo(0.0)
            trackX.snapTo(1.0)
            trackPress.snapTo(1.0)
            glow.snapTo(0.0)
